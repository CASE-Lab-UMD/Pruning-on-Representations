...
def get_llm(model_name, cache_dir="llm_weights"):
    ...
    # model.seqlen = model.config.seqlen 
    return model
...
def main():
    ...
    parser.add_argument("--prune_method", type=str, choices=["magnitude", "wanda", "sparsegpt", "neuron_partition", 
                        "ablate_mag_seq", "ablate_wanda_seq", "ablate_mag_iter", "ablate_wanda_iter", "search"])
    parser.add_argument("--cache_dir", default="llm_weights", type=str )
    parser.add_argument('--use_variant', action="store_true", help="whether to use the wanda variant described in the appendix")
    parser.add_argument('--save', type=str, default=None, help='Path to save results.')
    parser.add_argument('--save_model', type=str, default=None, help='Path to save the pruned model.')
    parser.add_argument('--seqlen', type=int, default=512, help='Sequence length for evaluation.')
    ...
    if args.sparsity_ratio != 0:
        ...
        if args.prune_method == "wanda":
            prune_wanda(args, model, tokenizer, device, prune_n=prune_n, prune_m=prune_m, seqlen=seqlen)
        elif args.prune_method == "magnitude":
            prune_magnitude(args, model, tokenizer, device, prune_n=prune_n, prune_m=prune_m, seqlen=seqlen)
        elif args.prune_method == "sparsegpt":
            prune_sparsegpt(args, model, tokenizer, device, prune_n=prune_n, prune_m=prune_m, seqlen=seqlen)
        elif "ablate" in args.prune_method:
            prune_ablate(args, model, tokenizer, device, prune_n=prune_n, prune_m=prune_m, seqlen=seqlen)
        elif "neuron_partition" in args.prune_method: 
            neuron_partition(args, model, tokenizer, device, prune_n=prune_n, prune_m=prune_m, seqlen=seqlen)
